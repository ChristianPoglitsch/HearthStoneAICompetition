...
namespace SabberStoneBasicAI.PartialObservation
{
	class POGameHandler
	{
		...
		public bool PlayGame(bool addToGameStats = true, bool debug = false)
		{
			...
			GPTAgent agent = null;
			if (player1 is GPTAgent)
			{
				agent = (GPTAgent)player1;
				agent.FinalizeGame(game.Player1.PlayState);
				player2.FinalizeGame();
				return true;
			}
			if (player2 is GPTAgent)
			{
				agent = (GPTAgent)player2;
				agent.FinalizeGame(game.Player2.PlayState);
