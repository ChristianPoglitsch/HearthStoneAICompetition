...
using SabberStoneAICompetition.src;
...
namespace SabberStoneBasicAI
{
	internal class Program
	{
		...
		private static void Main()
		{
			...
			//FullGame();
			//RandomGames();
			//TestPOGamesMulti(nr_of_threads: 2, games_per_thread: 1);
			TestPOGame(2);
			//TestFullGames();
			//TestTournament();
		...
		private static void TestPOGamesMulti(int nr_of_threads, int games_per_thread)
		{
			Thread[] threads = new Thread[nr_of_threads];
			for (int i = 0, sz = threads.Length; i < sz; i++)
			{
				threads[i] = new Thread(() => TestPOGame(games_per_thread));
				threads[i].Start();
			}
			for (int i = 0, sz = threads.Length; i < sz; i++)
			{
		...
		public static void TestTournament()
		{
			...
			agents[0] = new Agent(typeof(GPTAgent), "GPT Agent");
			agents[1] = new Agent(typeof(RandomAgent), "Greedy Agent");
		...
		public static void TestPOGame(int nr_of_games)
		{
			Console.WriteLine("Setup gameConfig");
			Console.WriteLine("Setup POGameHandler");

			//var agent = new GPTAgent(createLogFiles: true, logPath: "logs", filename: "random", folder: "random");
			var agent = new GPTAgent(createLogFiles: true, logPath: "logs", filename: "greedy", folder: "greedy");
			agent.SetCompletionOptions(temperature: 0.8f, freqPenalty: 0.0f);
			AbstractAgent player1 = agent;
			//AbstractAgent player1 = new GPTAgent("http://localhost:4891/v1", "Llama 3 8B Instruct", true);
			//AbstractAgent player2 = new RandomAgent();
			AbstractAgent player2 = new GreedyAgent();

			var configList = ProgramHelper.GetGameConfigs();

			foreach (var gameConfig in configList)
			{
				var gameHandler = new POGameHandler(gameConfig, player1, player2, repeatDraws: false);

				Console.WriteLine("Simulate Games");
				gameHandler.PlayGames(nr_of_games: nr_of_games, addResultToGameStats: true, debug: false);
